...
import { getTags, postTag } from './utils/api';
  ...
  return (
    <div className="lg:container mx-20 my-20">
      ...
      <div className="mx-20 flex flex-wrap">
        <EmptyTagCard addNewHandler={setOpenNewTag} />
        {tags.map((tag, index) =>
          <TagCard key={index} tag={tag} color={getColor(index)} />
        )}
      </div>
      <Popup open={openNewTag} closeOnDocumentClick modal onClose={() => setOpenNewTag(false)}>
        <div className="p-5 rounded-lg">
          ...
          <div>
            <Input
              className="py-2"
              label="Title" placeholder="Oasis - Wonderwall"
              value={newTag.title}
